...
integer :: useNoise,runSPL,runMarine,doEscarpment,useAdvect3d_lag
...
double precision :: sigma, mu, amplitude,paleotopo
...
! Inclined surface
!if (useNoise) call random_number (h)
!do j=1,ny
!  do i=1,nx
!    ij   = (j-1)*nx+i
!    xpos = (i-1)*dx
!    if (useNoise) then
!      h(ij) = (h(ij)-0.5d0)*0.25d0 + 100 - xpos*1e-3
!    else
!      h(ij) = 100 - xpos*1e-3
!    endif
!  enddo
...
do j=1,ny
  do i=1,nx
    ...
    xpos = (i-1)*dx
    if (useNoise) then
      if (doEscarpment) then
        if (xpos<=mu) then
          h(ij) = (h(ij)-0.5d0)*0.25d0 + amplitude + paleotopo
        else
          h(ij) = (h(ij)-0.5d0)*0.25d0 + amplitude*exp((-1.d0*(xpos-mu)**2)/(2*sigma**2)) + paleotopo
        endif
      else
        h(ij) = (h(ij)-0.5d0)*0.25d0 + amplitude*exp((-1.d0*(xpos-mu)**2)/(2*sigma**2)) + paleotopo
      endif
      ...
      if (doEscarpment) then
        if (xpos<=mu) then
          h(ij) = amplitude + paleotopo
        else
          h(ij) = amplitude*exp((-1.d0*(xpos-mu)**2)/(2*sigma**2)) + paleotopo
        endif
      else
        h(ij) = amplitude*exp((-1.d0*(xpos-mu)**2)/(2*sigma**2)) + paleotopo
      endif
